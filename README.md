...
3. Prove the fundamental theorem of symmetric functions. The finite variables case is already done in `mathlib`.
...
7. (Later?) Refactor `MvPowerSeries` to use the total monoid algebra construction. The monoid here is $(M,+)=\big((\mathbb{Z}_{\geq 0})^I,+\big)$ where $I$ represents the index set for the variables. Various theoretical aspects of the power series ring fit into this generalized setting.
    * A monoid homomorphism $M \to N$ induces a homomorphism $R[[M]]\to R[[N]]$. An example of this in power series (or polynomial) ring setting occurs when one sends some of the variables to zero. 
    * A group action on $M$ lifts to an action on $R[[M]]$. For example, for power series (or polynomials) we have an action of the symmetric group on the variables. The subset consisting of elements that are invariant under the action form a subalgebra by general principles.
