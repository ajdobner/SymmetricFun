...
/-- A generic, un-sized integer partition, represented as a multiset of positive integers. -/
@[ext]
structure GenericPartition where
  /-- positive integers forming the partition -/
  parts : Multiset ℕ
  /-- proof that the `parts` are positive -/
  parts_pos : ∀ {i}, i ∈ parts → 0 < i
deriving DecidableEq

namespace GenericPartition

attribute [grind →] parts_pos

/-- The size of a generic partition. -/
def size (p : GenericPartition) : ℕ :=
  p.parts.sum

-- TODO: Need to add symmetric function specific API here,
-- like `length`, transpose/conjugate, etc.

end GenericPartition

/-- A partition of `n` is a multiset of positive integers summing to `n`. -/
@[ext]
structure Partition (n : ℕ) extends GenericPartition where
...
theorem ofComposition_surj {n : ℕ} : Function.Surjective (ofComposition n) := by sorry
  -- rintro ⟨b, hb₁, hb₂⟩
...
@[simp] lemma ofSym_map (e : σ ≃ τ) (s : Sym σ n) :
    ofSym (s.map e) = ofSym s := by sorry
